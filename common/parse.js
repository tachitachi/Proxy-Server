...
define(function(require){
	...
	function GetTime(inputTime){
		...
		var month = bufutil.padDigits(curtime.getMonth() + 1, 2); //zero index
		var day = bufutil.padDigits(curtime.getDate(), 2);
		var year = curtime.getYear() + 1900;
		var hours = bufutil.padDigits(curtime.getHours(), 2);
		var minutes = bufutil.padDigits(curtime.getMinutes(), 2);
		var seconds = bufutil.padDigits(curtime.getSeconds(), 2);
		var milliseconds = bufutil.padDigits(curtime.getMilliseconds(), 3);
	...
	function AsciiToHex(str) {
	    ...
	    for (var i = 0; i < str.length; i++){
	        msg.push(bufutil.printHex(str.charCodeAt(i)));
		}
		if(msg[msg.length - 1] != '00'){
			msg.push(bufutil.printHex(0));
		}
	    return msg;
	}
	...
	function IntToHexString(num, byteCount){
		...
		for(var i = 0; i < byteCount; i++){
			bytes.push(bufutil.printHex((input >> (8 * i)) & 0xff));
		}
	...
	function DecimalToHex(msg){
		...
		for(var i in msg){
			bytes.push(bufutil.printHex(msg[i]));
		}
