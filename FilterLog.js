...
process.argv.forEach(function (val, index, array) {
	...
	if(index == 2){
		...
		outlogfile = path.join('logs', 'filtered', val);
		...
		for(var i = 0; i < packets.length; i++){
			...
			case 0x0003:
				...
				for(var packetIdx = 0; packetIdx < recvPackets.length; packetIdx++){
                    ...
                    if(recvPackets[packetIdx].header == 0x01c8){
                        var itemId = recvPackets[packetIdx].data[RECV[recvPackets[packetIdx].header].datamap.itemId.index].value;
                        var ID = recvPackets[packetIdx].data[RECV[recvPackets[packetIdx].header].datamap.ID.index].value;
                        
                        if(itemId == 11548 && ID == 4768005){
                            logText += packetText;
                        }
                    }
                    else if(recvPackets[packetIdx].header == 0x07fb){
                        var sourceId = recvPackets[packetIdx].data[RECV[recvPackets[packetIdx].header].datamap.sourceId.index].value;
                        var targetId = recvPackets[packetIdx].data[RECV[recvPackets[packetIdx].header].datamap.targetId.index].value;
                        var skillId = recvPackets[packetIdx].data[RECV[recvPackets[packetIdx].header].datamap.skillId.index].value;
                        
                        if(sourceId == 3802723 && targetId == 4768005 && skillId == 5033){
                            logText += packetText;
                        }
                    }
